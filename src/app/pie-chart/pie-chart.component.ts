...
export class PieChartComponent implements OnInit {
  ...
  Confirmed = this.array.map(e => e["Confirmed"]);
  increase = this.array.map(e=> e["Increase rate"])
  Recovered = this.array.map(e=> e["Recovered"])
  Death = this.array.map(e => e["Deaths"])


  public pieChartLabels: Label[] = ['Confirmed','Recovered','Deaths']
  public pieChartData: number[] = [this.Confirmed.slice(-1)[0], this.Recovered.slice(-1)[0], this.Death.slice(-1)[0]];
  ...
  ngOnInit(): void {
    //console.log(this.Confirmed.slice(-1)[0]);
